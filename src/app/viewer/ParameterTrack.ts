 ...
 */
import {Component, Input, Inject, OnInit, ViewChildren, QueryList, ViewChild, ElementRef} from "@angular/core";
import {IParameterTrackModel} from "../backend/IParameterTrackModel";
import {PaintManager} from "./PaintManager";
import {MarkerDirective} from "./MarkerDirective";
...
<div class="parameterTrack">
        <svg class="marker" [attr.viewBox]="viewBox"
        ...
        (mouseover)="onMouseOver()" (mouseout)="onMouseOut()" (mousemove)="onMouseMove($event)"/>
        <circle *ngIf="getDrawMarker()" [attr.cx]="markerX" [attr.cy]="markerY" r="2" fill="red"/>
        </svg>
        </div>
    `,
...
export class ParameterTrack implements OnInit {
  ...
  @ViewChild(MarkerDirective) child: MarkerDirective;
  ...
  onMouseMove(event: MouseEvent) {
    this.drawMarker = true;
    var childRect = this.child.getHTMLElement().getBoundingClientRect();

    this.markerY = Math.floor(100 * (event.clientY - childRect.top) / (childRect.bottom - childRect.top));
